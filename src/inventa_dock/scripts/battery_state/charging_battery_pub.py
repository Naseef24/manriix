...
In this case, the battery is a battery that is currently charging.
-------
Publishing Topics:
...
class BatteryStatePublisher:
    ...
    def __init__(self):
        ...
        self.voltage = 2.16  # Battery voltage level
        self.percentage = 0.24  # Percentage charge level
        self.power_supply_status = BatteryState.POWER_SUPPLY_STATUS_CHARGING  # Charging
        
        # Optional: Simulate charging progression
        self.charge_increment = 0.01  # 1% charge per interval
        
    def get_battery_state(self, event=None):
        """
        Callback function to report battery state at specific time interval.
        Simulates battery charging
        """
        # Simulate charging: increase percentage until fully charged
        if self.percentage < 1.0:
            self.percentage = min(self.percentage + self.charge_increment, 1.0)
            
            # Simulate voltage increase during charging
            self.voltage = min(self.voltage + 0.05, 4.2)
        else:
            # Fully charged
            self.power_supply_status = BatteryState.POWER_SUPPLY_STATUS_FULL
        ...
        rospy.loginfo(f"🔋 Charging: {self.percentage * 100:.2f}%, "
                      f"Voltage: {self.voltage:.2f}V, "
                      f"Status: {self.get_status_string()}")
    
    def get_status_string(self):
        """
        Convert power supply status to readable string
        """
        status_map = {
            ...
            BatteryState.POWER_SUPPLY_STATUS_FULL: "Fully Charged"
        }
        return status_map.get(self.power_supply_status, "Unknown")
...
#     self.voltage = 2.16 # Initialize the battery voltage level
#     self.percentage = 0.24  # Initialize the percentage charge level
#     self.power_supply_status = 1 # Initialize the power supply status (e.g. CHARGING)
