...
class HumanClusteringNode:
    ...
    def __init__(self):
        ...
        # self.scan_region_radius = 0.5  # Reasonable region radius in meters
        # self.scan_start_time = None
        # self.scan_duration = rospy.Duration(120)  # 2 minutes in seconds
#####Editeed 
        self.is_scanning = False
        self.scan_region_radius = 0.5 
        self.scan_start_time = None
        self.scan_duration = rospy.Duration(120)  
        self.robot_stopped = False
        self.last_position = None
        self.position_stable_duration = rospy.Duration(1.0)  
        self.last_position_time = None
        # self.scan_status_pub = rospy.Publisher('scan_status', String, queue_size=1)
   ######################   Recheck up      
        ...
        # self.scan_status_sub = rospy.Subscriber(
        #     'scan_status',
        #     String,
...
#########################################################3
    # def scan_status_callback(self, msg):
    #     self.scan_status = msg.data
    #     rospy.loginfo(f"Received scan status: {self.scan_status}")
...
####################################################################        
   ...
   ############## New chcek ##################3
        ...
        try:
            ...
            if self.is_scanning:
                ...
                if time_elapsed > self.scan_duration:
                    ...
                    start_scan_msg = String()
                    start_scan_msg.data = "no"
                    self.start_scan_pub.publish(start_scan_msg)
    ...
    #                             rospy.loginfo("Target reached, starting scan")
    #                             self.is_moving = False
    ...
    #                         if self.scan_status.lower() == 'scanning':
    #                             rospy.loginfo("Scan in progress" )    
                            
    #                         elif self.scan_status.lower() == 'not_scanning' and self.next_position_calculated:
...
#### Previous
    ...
    #                         # elif self.scan_status.lower() == 'not_scanning' and self.next_position_calculated:
    #                         # elif rospy.Duration() and self.next_position_calculated:
    #                         #     rospy.loginfo("scan complete, moving to next position")
    #                         #     self.current_optimal_position = self.next_position_calculated
    #                         #     self.next_position_calculated = None
    #                         #     self.target_reached = False
                ...
                try:
                    ...
                    if clusters:
                        ...
                        if not self.is_scanning:
                            ...
                            if optimal_positions:
                                rospy.loginfo(f"Top position: {optimal_positions[0]}")
                        else:
