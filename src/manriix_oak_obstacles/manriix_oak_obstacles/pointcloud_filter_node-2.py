...
class PointCloudFilterNode(Node):
    ...
    def __init__(self):
        ...
        # R/t are now looked up FRESH every callback (see
        # _update_transform's docstring-comment for why caching was
        # wrong once target_frame could be a moving frame like odom/map).
        self._R = None
        self._t = None
    ...
    def _update_transform(self, source_frame: str, stamp) -> bool:
        ...
        # stale cached transform silently produces world positions that
        # drift by exactly however far the robot has actually moved
        # since the transform was last (only) fetched. This was the
        # root cause of "stationary person marked as moving when the
        # robot moves" and the direction-scrambling symptom, and it
        # persisted across every target_frame swap because the caching
        # bug -- not the frame choice -- was the actual problem.
        #
        ...
        # even with fresh (non-cached) lookups. Falls back to "latest"
        # only if the exact-time lookup isn't available yet (e.g. TF
        # hasn't caught up), so we degrade gracefully instead of
