...
class POIManager(Node):
    ...
    def __init__(self):
        ...
        self.costmap_data: Optional[OccupancyGrid] = None   # ← ADD
        ...
        # INTEGRATE SOPHISTICATED POSITION EVALUATION
        # Load config from clustering_params.yaml (same file as human_clustering_node)
        # This ensures weights, formation_scores, and map params are all configurable
        ...
        if _clustering_config_path and os.path.exists(_clustering_config_path):
            try:
                ...
                # Unwrap ROS2 YAML wrapper if present
        ...
        # TRANSIENT_LOCAL: publisher keeps last message — new subscribers get it immediately
        # Required for /map which is published once at SLAM startup
        qos_map = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,
            durability=DurabilityPolicy.TRANSIENT_LOCAL,
            depth=1
        )
        # TRANSIENT_LOCAL: Nav2 costmap publisher latches — subscriber gets latest on connect
        ...
        # ADD: live costmap includes people as dynamic obstacles
        self.costmap_sub = self.create_subscription(
            OccupancyGrid,
            '/local_costmap/costmap',
            self.costmap_callback,
            qos_costmap #qos_best_effort
        )
    ...
    # def is_position_free(self, position: np.ndarray) -> bool:
    #     try:
    #         # Prefer live costmap (contains people) over static SLAM map
    #         map_to_check = self.costmap_data if self.costmap_data is not None else self.map_data

    # REPLACE WITH:
    def is_position_free(self, position: np.ndarray, static_only: bool = False) -> bool:
        try:
            # static_only=True: use SLAM map only (permanent walls)
            # static_only=False: prefer live costmap (people + walls)
