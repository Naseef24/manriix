...
#             ClusterTrackArray, self.output_tracks_topic, qos)
#         self.pub_markers = self.create_publisher(
#             MarkerArray, self.output_markers_topic, qos)
...
#         for trk in self.tracks.values():
#             # Only publish tracks seen this frame (missed_frames == 0) as
#             # solid detections; still-alive-but-missed tracks are kept
#             # internally (for occlusion survival) but not spammed to viz.
#             ct = ClusterTrack()
#             ct.track_id = trk.track_id
#             ct.centroid = Point(x=float(trk.centroid[0]),
#                                  y=float(trk.centroid[1]),
