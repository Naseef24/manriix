...
class PointCloudFilterNode(Node):
    ...
    def __init__(self):
        ...
        # R/t are looked up FRESH every callback (see _update_transform's
        # docstring for why permanent caching was wrong once target_frame
        # became a moving frame like odom/map). A bounded, aged fallback
        # to the last-known-good transform is still allowed for
        # TRANSIENT delivery delays (e.g. the machine publishing TF is
        # briefly CPU-starved) -- see max_stale_transform_age below.
        self._R = None
        self._t = None
        self._last_good_stamp = None  # sim-time seconds of last successful lookup

        self.declare_parameter('max_stale_transform_age', 0.5)  # [s]
        self.max_stale_transform_age = float(
            self.get_parameter('max_stale_transform_age').value)
    ...
    # ------------------------------------------------------------------
    def _update_transform(self, source_frame: str, stamp) -> bool:
        # IMPORTANT: this must run on EVERY callback, not just once and
        # cache. Caching after the first lookup is only valid when
        # target_frame is a frame RIGIDLY attached to the camera (like
        # base_footprint -- the mount never moves relative to the robot
        # body). The moment target_frame is odom or map, this transform
        # genuinely changes every timestep as the robot drives -- a
        # PERMANENTLY stale cached transform silently produces world
        # positions that drift by exactly however far the robot has
        # actually moved since the transform was last (only) fetched.
        # This was the root cause of "stationary person marked as moving
        # when the robot moves" and the direction-scrambling symptom.
        #
        # Also: look up the transform AT THE POINT CLOUD'S OWN CAPTURE
        # TIME (msg.header.stamp), not "latest available"
        # (rclpy.time.Time()). During robot motion, "latest" is always
        # slightly AHEAD of when the data was actually captured (by
        # however long this callback took to get scheduled), which
        # introduces a small but real, motion-correlated position error
        # even with fresh (non-cached) lookups.
        #
        # Below that: a BOUNDED, AGED fallback to the last successful
        # lookup, for TRANSIENT delivery delay only (e.g. the machine
        # publishing TF -- robot_state_publisher / cuVSLAM -- is
        # momentarily CPU-starved by something unrelated). Unlike the
        # original bug, this is (a) always attempted fresh first, (b)
        # explicitly age-limited via max_stale_transform_age, and (c)
        # logged loudly, so a genuinely stuck/slow TF source causes a
        # visible warning and eventually dropped frames -- not silent
        # unbounded drift.
        now_sec = Time.from_msg(stamp).nanoseconds * 1e-9
        try:
            ...
            try:
                ...
                self.get_logger().warn(
                    'TF at exact capture time unavailable, using latest '
                    '(may be slightly stale during fast motion)',
                    throttle_duration_sec=5.0)
            except TransformException as ex:
                if (self._R is not None and self._last_good_stamp is not None
                        and (now_sec - self._last_good_stamp) <= self.max_stale_transform_age):
                    self.get_logger().warn(
                        f'TF {source_frame} -> {self.target_frame} unavailable '
                        f'(likely a transient delay on the machine publishing TF) '
                        f'-- reusing last-known transform, '
                        f'{now_sec - self._last_good_stamp:.2f}s old '
                        f'(max allowed: {self.max_stale_transform_age}s)',
                        throttle_duration_sec=1.0,
                    )
                    return True  # reuse self._R/self._t as-is
                self.get_logger().warn(
                    f'TF {source_frame} -> {self.target_frame} not available '
                    f'and no usable recent transform to fall back on '
                    f'(dropping this frame): {ex}',
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        if not self._update_transform(source_frame, msg.header.stamp):
            return
