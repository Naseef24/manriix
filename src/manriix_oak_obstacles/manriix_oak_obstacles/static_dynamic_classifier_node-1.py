...
#             MarkerArray, self.get_parameter('output_markers_topic').value, qos)
...
#         ref_dense_tree = cKDTree(ref_frame.dense_pts)
...
#             pts_cam = (ref_frame.dense_pts - cam_pos) @ R_cw.T
#             depth = pts_cam[:, 2]
#             valid = depth > 1e-3
#             if np.any(valid):
...
#                 ref_dense_valid_pts = ref_frame.dense_pts[valid]
#             if ref_frame.labeled_pts.shape[0] > 0:
#                 ref_labeled_tree = cKDTree(ref_frame.labeled_pts)
...
#         if ref_pixel_tree is not None and cam_pos is not None:
...
#             trk.points, field_names=('x', 'y', 'z'), skip_nans=True).astype(np.float64)
#         if pts.shape[0] == 0:
#             mins = maxs = np.array([trk.centroid.x, trk.centroid.y, trk.centroid.z])
#         else:
#             mins = pts.min(axis=0)
#             maxs = pts.max(axis=0)
