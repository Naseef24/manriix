...
class HumanClusteringNode(Node):
    ...
    def __init__(self):
        ...
        # Subscribers created last — all state is ready before any callback can fire
        self.initialize_subscribers()
    ...
    def try_fallback_positioning(self):
        ...
        try:
            ...
            # Fallback positions are provided by POI Manager via /mission/pois.
            # This node no longer calculates them — reset recovery and let
            # POI Manager drive the next position.
            self.get_logger().warn(
                    ...
                    try:
                        ...
                        if map_pos and len(map_pos) > 0:
                            ...
                            if abs(mx) > 1000 or abs(my) > 1000:
                                ...
                                continue
                            # ADD: distance filter in parallel path
                            if self.robot_position is not None:
                                max_dist = self.config.get('detection', {}).get(
                                    'effective_range', {}).get('max_detection_distance', 10.0)
                                human_dist = np.sqrt((mx - self.robot_position[0])**2 +
                                                    (my - self.robot_position[1])**2)
                                if human_dist > max_dist:
                                    continue

                            # ADD: distance-from-robot filter — reject humans beyond effective range
                            if self.robot_position is not None:
                                max_dist = self.config.get('detection', {}).get(
                                    'effective_range', {}).get('max_detection_distance', 10.0)
                                human_dist = np.sqrt((mx - self.robot_position[0])**2 +
        ...
        try:
            if not self._init_ok:
                self.get_logger().error(
                    "Initialization failed — node is inactive. Check config_path and logs."
                )
                return        
