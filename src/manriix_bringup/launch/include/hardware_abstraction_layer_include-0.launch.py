...
RPLidar A2M12 is physically attached to the Kyro Jetson (not the main
Jetson this bringup runs on), so it is NOT launched locally here. It is
started/stopped remotely over SSH via a systemd service on Kyro, using
the same ownership pattern as the OAK-D camera in perceptor_include.
================================================================================
"""
...
def stop_remote_rplidar_service(context, *args, **kwargs):
    """Stop the RPLidar systemd service running on Kyro."""

    print(
        "[RPLIDAR REMOTE] Stopping manriix-rplidar.service on Kyro..."
    )
    ...
    try:
        ...
        if result.returncode == 0:
            print(
                "[RPLIDAR REMOTE] "
                "manriix-rplidar.service stopped successfully."
            )
        else:
            print(
                "[RPLIDAR REMOTE] WARNING: remote stop returned "
                f"code {result.returncode}"
            )

            if result.stderr:
                print(
                    "[RPLIDAR REMOTE] stderr: "
                    f"{result.stderr.strip()}"
                )

    except subprocess.TimeoutExpired:
        print(
            "[RPLIDAR REMOTE] ERROR: timeout while stopping "
            "manriix-rplidar.service."
        )

    except Exception as exc:
        print(
            "[RPLIDAR REMOTE] ERROR while stopping remote "
            f"RPLidar service: {exc}"
        )
...
def generate_launch_description():
    ...
    enable_rplidar_arg = DeclareLaunchArgument(
        ...
        default_value='True',
        description=(
            'Enable RPLidar A2M12 2D laser scanner. Hardware lives on '
            'Kyro; this starts/stops manriix-rplidar.service over SSH '
            'rather than launching a local node.'
        ),
    )
    ...
    # ================================================================
    # RPLidar A2M12 (remote — lives on Kyro)
    # ================================================================
    #
    # IMPORTANT:
    #
    # The RPLidar is physically attached to the Kyro Jetson, not this
    # machine. It is NOT launched as a local node/IncludeLaunchDescription
    # here. Instead, following the same ownership pattern used for the
    # OAK-D camera in perceptor_include.launch.py, this starts and stops
    # a systemd service (manriix-rplidar.service) on Kyro over SSH.
    #
    # manriix-rplidar.service on Kyro is expected to run the equivalent
    # of:
    #
    ...
    rplidar_enabled_log = LogInfo(
        condition=IfCondition(enable_rplidar),
        msg=(
            "[HARDWARE LAYER] RPLidar A2M12 enabled. "
            "Lidar hardware remains managed remotely on Kyro via "
            "manriix-rplidar.service."
        ),
    )

    rplidar_remote_start = ExecuteProcess(
        cmd=[
            "ssh",
            "manriix-kyro",
            "sudo",
    ...
    rplidar_shutdown_handler = RegisterEventHandler(
        OnShutdown(
            on_shutdown=[
                LogInfo(
                    msg=(
                        "[RPLIDAR REMOTE] Main launch shutting down. "
                        "Stopping manriix-rplidar.service on Kyro..."
                    )
                ),
                OpaqueFunction(
                    function=stop_remote_rplidar_service
                ),
            ]
        ),
        condition=IfCondition(enable_rplidar),
    ...
    return LaunchDescription([
        ...
        LogInfo(msg=[
            ...
            '  RPLidar A2M12:      ', enable_rplidar, ' (remote, on Kyro)\n',
        ...
        rplidar_enabled_log,
        rplidar_remote_start,
        rplidar_shutdown_handler,
