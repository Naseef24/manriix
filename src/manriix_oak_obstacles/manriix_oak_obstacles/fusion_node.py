...
class FusionNode(Node):
    ...
    def _publish(self, msg: ClusterTrackArray, now):
        ...
        for trk in msg.tracks:
            ...
            current_track_ids.add(trk.track_id)

            # IMPORTANT: marker id = the track's own persistent track_id,
            # NOT a loop counter. RViz/Foxglove key a marker's identity on
            # (namespace, id); if id instead came from position-in-loop,
            # any OTHER track appearing/disappearing anywhere in the scene
            # shifts every subsequent track's assigned id -- which looks
            # exactly like the marker silently stopping updates (then
            # fading out after its lifetime) while a "new" id pops in at
            # the same spot. Tying id to track_id makes each track's
            # marker identity stable across frames regardless of what
            # else is happening elsewhere in the scene.
            for shape_marker in self._make_shape_markers(msg.header, trk):
                shape_marker.id = trk.track_id
                marker_array.markers.append(shape_marker)
            marker_array.markers.append(
                self._make_label(msg.header, trk, trk.track_id))

        # Clean up markers for tracks that existed last frame but not this
        # one. Blanket-delete all possible namespaces for each dropped
        # track_id -- deleting a (ns,id) that was never added is a no-op,
        # so this is safe even though a track only ever used SOME of these
        # namespaces (e.g. non-person tracks never had fused_person_box).
        dropped = self._prev_track_ids - current_track_ids
        for tid in dropped:
            for ns in ('fused_person_points', 'fused_person_box',
                      'fused_voxels', 'fused_labels'):
                m = Marker()
                m.header = out.header
                m.ns = ns
                m.id = tid
