...
All three cameras must remain owned by multi_zed_capture_node.py.
...
def stop_remote_oak_service(context, *args, **kwargs):
    """Stop the OAK-D systemd service running on Kyro."""
    ...
    try:
        ...
        if result.returncode == 0:
            print(
                "[OAK REMOTE] "
                "manriix-oak.service stopped successfully."
            )
        else:
            print(
...
def generate_launch_description():
    ...
    multi_zed_capture = Node(
        ...
        executable="multi_zed_capture_node.py",
