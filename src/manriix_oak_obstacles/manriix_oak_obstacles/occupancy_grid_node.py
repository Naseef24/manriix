...
class OccupancyGridNode(Node):
    ...
    # ------------------------------------------------------------------
    def _get_frame_transform(self, source_frame):
        """R, t such that map_point = R @ local_point + t. Cluster tracks
        arrive in a ROBOT-RELATIVE frame (base_footprint, per the filter
        node's target_frame) -- they must be rotated+translated into the
        world-fixed map_frame before being written into the PERSISTENT
        grid. Skipping this was invisible on the bench, where map->odom->
        base_footprint were identity static TFs (so base_footprint ==
        map numerically); it becomes visible the moment the robot
        actually moves/rotates (e.g. once cuVSLAM is driving TF), because
        every obstacle then gets rasterized at its robot-relative
        coordinates instead of its true world position.
        """
        ...
        try:
            ...
            self.get_logger().warn(
                f'grid: TF {source_frame}->{self.map_frame} unavailable, '
                f'skipping frame (obstacles would be mis-placed): {ex}',
    ...
    def _cb(self, msg: ClusterTrackArray):
        ...
        # All tracks in one ClusterTrackArray share the same source frame
        # (whatever the filter node's target_frame was, e.g.
        # base_footprint) -- look the transform up once per callback.
        tf_result = self._get_frame_transform(msg.header.frame_id)
        if tf_result is None:
            return  # can't safely place anything in the world grid this frame
        R, t = tf_result
        ...
        for trk in msg.tracks:
            ...
            if pts.shape[0] == 0:
                continue
            # Transform points into the world-fixed map frame.
            pts_map = pts @ R.T + t
            xy = pts_map[:, :2]
            ...
            if trk.classification == 'static':
                ...
                # footprint along v*t. Velocity is a VECTOR: rotate only,
                # do not translate.
                v_local = np.array([trk.velocity.x, trk.velocity.y, 0.0])
                v_map = (R @ v_local)[:2]
                steps = max(1, self.predict_steps)
                for k in range(steps + 1):
                    tt = self.predict_horizon * k / steps
                    swept = xy + v_map * tt
