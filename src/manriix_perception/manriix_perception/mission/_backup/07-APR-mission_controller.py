...
class MissionController(Node):
    ...
    def __init__(self):
        ...
        # [TELEOP] Manual override
        self._teleop_paused = False 
        # [PHOTO] Camera active awareness
        ...
        self._last_nav_goal_time = 0.0  # Timestamp of last Nav2 goal sent (rate limiting)        
        self._recovery_cooldown_until = 0.0  # Don't accept targets until this time        
        ...
        # [TELEOP] Manual override subscriber
        ...
        # [PHOTO] Camera command awareness (from RC controller or PhotoIntelligenceSystem)
        ...
        # Service — toggle autonomous navigation at runtime without restart
        # Usage: ros2 service call /mission_controller/set_autonomous std_srvs/srv/SetBool "{data: true}"
        self.set_autonomous_srv = self.create_service(
            SetBool,
            '/mission_controller/set_autonomous',
            self._handle_set_autonomous,
            callback_group=self.cb_group)
        ...
        try:
            ...
            if msg.data and not self._teleop_paused:
                ...
                if self._camera_active:
                    ...
                    self._camera_active = False
                    self.get_logger().info("📷 Camera stopped by teleop override")
                self._reset_retry_state()
                self._failed_positions = []
                self.current_target = None
                self.available_targets = []
    ...
    def photo_command_callback(self, msg: String):
        """
        [PHOTO] Track when camera is active.
        
        Triggered by either:
          - PhotoIntelligenceSystem (autonomous photo session)
          - RC controller SA switch (manual photo trigger)
        
        When camera is active, robot should NOT move.
        """
        try:
            if msg.data == "yes":
                self._camera_active = True
                self.get_logger().info("📷 Camera ACTIVE — holding position")
                # Cancel any active navigation — don't move while shooting
                if self.current_nav_goal_handle is not None:
                    self._cancel_navigation()
            ...
            #     self.get_logger().info("📷 Camera STOPPED")
                ...
                if (self.current_state == MissionState.POSITIONING_OPTIMAL
                    ...
                    and self.current_nav_goal_handle is None):
                    self.get_logger().info("📷 Camera cleared — re-sending Nav2 goal")
    ...
    def _execute_recovery_command(self, command: str):
        ...
        try:
            ...
            #     self._is_recovery_action_active = False
                ...
                # Level 2 recovery — sensor reset is handled at the hardware/driver
                # level externally. Mission controller acknowledges and clears state
                # so recovery_manager can proceed to next level if needed.
...
def main(args=None):
    ...
    #         stop_msg.data = "no"
    #         node.photo_command_pub.publish(stop_msg)
    #         node.get_logger().info("📷 Camera stop sent on shutdown")
    #     except Exception:
    #         pass
    #     node.destroy_node()
    #     rclpy.shutdown()
    ...
    # finally:
    #     # Stop navigation and camera on shutdown
    #     try:
    #         # Cancel any active Nav2 goal
    #         if node.current_nav_goal_handle is not None:
    #             node.current_nav_goal_handle.cancel_goal_async()
    #             node.get_logger().info("🛑 Nav2 goal cancelled on shutdown")
    #         # Stop camera
    #         stop_msg = String()
    #         stop_msg.data = "no"
    #         node.photo_command_pub.publish(stop_msg)
    ...
    #         node.photo_command_pub.publish(stop_msg)
    #         node.get_logger().info("📷 Camera stop sent on shutdown")
    #         time.sleep(0.5)  # Allow DDS to deliver to camera driver
    #     except Exception:
    #         pass
    #     node.destroy_node()
    #     rclpy.shutdown()
