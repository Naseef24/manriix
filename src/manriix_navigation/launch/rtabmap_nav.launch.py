...
from launch.actions import DeclareLaunchArgument, SetEnvironmentVariable, LogInfo, OpaqueFunction, TimerAction, GroupAction, IncludeLaunchDescription
...
def generate_launch_description():
    ...
    # ─── Nav2 paths and lifecycle nodes (added by integration step 4a) ────
    pkg_navigation = get_package_share_directory('manriix_navigation')
    nav2_params_path = os.path.join(
        pkg_navigation, 'config', 'nav2', 'nav2_params_rtabmap.yaml'
    )

 # ─── Nav2 launch action (sub-step 4b — final, corrected) ──────────────
    #
    # APPROACH: Mirrors your working slam_nav.launch.py
    # - Uses nav2_bringup's navigation_launch.py (well-tested, official path)
    # - Adds collision_monitor as a separate node + lifecycle manager
    #   (collision_monitor is NOT in nav2_bringup's default lifecycle list)
    #
    # cmd_vel topic chain (matches your twist_mux config):
    #
    #   Nav2 (controller_server) publishes → /cmd_vel
    #                                             ↓
    #   collision_monitor (in: cmd_vel) → publishes → /cmd_vel_safe
    #                                             ↓
    #                            twist_mux (navigation slot, priority 5)
    #                                             ↓
    #                            /manriix_4ws_controller/cmd_vel
    #                                             ↓
    #                                          wheels
    #
    # behavior_server (recovery: spin, wait) publishes → cmd_vel by default.
    # If you want recovery to bypass collision_monitor, remap it to
    # /cmd_vel_recovery (twist_mux priority 8) — not done here to keep
    # behavior identical to slam_nav.launch.py's setup.
    #
    # References:
    ...
    #
    # Why TimerAction(period=70.0):
    #   RTAB-Map needs to publish /map before Nav2's static_layer subscribes.
    #   RTAB-Map starts at t=60s (in this launch's rtabmap_launch timer),
    #   so Nav2 at t=70s gives 10s headroom for /map to start publishing.
    #
    # Why launch_nav2 flag:
    #   Allows mapping-only mode (launch_nav2:=false) where you just want
    #   RTAB-Map running without Nav2 overhead.
            ...
            GroupAction(
                ...
                actions=[
                    LogInfo(msg=[
                        "\n",
                        "=" * 64, "\n",
                        "[Nav2] Launching navigation stack (via nav2_bringup)\n",
                        "  Config: ", nav2_params_path, "\n",
                        "  Map source:  /map (from RTAB-Map)\n",
                        "  Odom source: /odometry/filtered (from EKF)\n",
                        "  Output:      /cmd_vel_safe → twist_mux (nav slot, prio 5)\n",
                        "=" * 64,
                    ]),

                    # ─── Nav2 stack via official nav2_bringup ─────────────
                    # This launches: controller_server, planner_server,
                    # behavior_server, bt_navigator, waypoint_follower,
                    # velocity_smoother, smoother_server, and a
                    # lifecycle_manager that auto-activates them all.
                    #
                    # collision_monitor is NOT included by nav2_bringup —
                    # we add it separately below.
                    ...
                    # ─── Collision Monitor (separate lifecycle) ───────────
                    # Reads cmd_vel from Nav2, outputs cmd_vel_safe to
                    # twist_mux's navigation slot. Stops/slows the robot
                    # via polygon checks against pointcloud + scan sources.
                    #
                    # cmd_vel_in_topic and cmd_vel_out_topic are configured
                    # in nav2_params_rtabmap.yaml under collision_monitor.
                    Node(
                    ...
                    # ─── Lifecycle manager for collision_monitor ──────────
                    # Separate manager because nav2_bringup's manager
                    # doesn't include collision_monitor in its node list.
                    # bond_timeout: 4.0 matches slam_nav.launch.py.
                    Node(
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('launch_nav2', default_value='true', description='Launch Nav2 navigation stack on top of RTAB-Map.'),
        ...
        DeclareLaunchArgument('database_path',  default_value='~/manriix_rtabmap.db',  description='Where is the map saved/loaded.'),
        ...
        # DeclareLaunchArgument('rtabmap_args',   default_value='-d --Reg/Force3DoF true',                   description='Backward compatibility, use "args" instead.'),
        DeclareLaunchArgument(
                                ...
                                description='RTAB-Map internal parameters via command-line args. '
                                            'See https://github.com/introlab/rtabmap/wiki/Parameters'
                            ),
        ...
        DeclareLaunchArgument('approx_sync_max_interval',  default_value='0.2', description='(sec) 0 means infinite interval duration (used with approx_sync=true)'),

        # RGB-D related topics
        DeclareLaunchArgument('rgb_topic',           default_value='/oak_d_pro_w/rgb/image_raw',       description=''),
        DeclareLaunchArgument('depth_topic',         default_value='/oak_d_pro_w/stereo/image_raw', description=''),
        DeclareLaunchArgument('camera_info_topic',   default_value='/oak_d_pro_w/rgb/camera_info',            description=''),
        ...
        DeclareLaunchArgument('scan_normal_k',        default_value='10',           description=''),
        ...
        DeclareLaunchArgument('odom_tf_angular_variance',   default_value='0.05',    description='If TF is used to get odometry, this is the default angular variance'),
        DeclareLaunchArgument('odom_tf_linear_variance',    default_value='0.01',   description='If TF is used to get odometry, this is the default linear variance'),
